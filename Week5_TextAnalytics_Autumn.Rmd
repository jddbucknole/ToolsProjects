...
2. Count how many words are in each book and make a line chart with book name (or number) on the x-axis and the number of words on the y. Note: if you use the book number, you will need to add an aesthetic `group = 1`. This graph should have the books in chronological order.  Make sure to have appropriate labels/titles.  
...
2. You should notice that characters names appear more frequently that other words.  Add the characters names to the stop_words list and re-plot the top 10 words without character names.  The code below contains a list of characters.  Feel free to add to it.
